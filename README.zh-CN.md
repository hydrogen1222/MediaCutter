...
- **电台视频与硬字幕压制**：用封面图+音频合成视频，或将 `.ass` 字幕硬压到视频中。压制时自动检测并优先使用 GPU 硬件编码（NVENC / QuickSync / AMF / VAAPI），无可用 GPU 时回退到 libx264（`-preset veryfast`，调用全部 CPU 核心）。当前使用的编码器会显示在进度条状态中。若检测到的硬件编码器通过了探测却在实际压制时卡住（例如某些 AMF/驱动组合会一直停在 0%、CPU/GPU 均无占用），会自动放弃并改用 libx264 重试，导出绝不会一直卡死。
