...
- **Radio Video & Subtitle Burn**: Turn a cover image + audio into a video, or hardcode `.ass` subtitles into a video. Encoding automatically uses GPU hardware acceleration (NVENC / QuickSync / VAAPI) when a usable encoder is detected, and falls back to libx264 (`-preset veryfast`, all CPU cores) otherwise. The active encoder is shown in the progress line. AMF is intentionally skipped: on some AMD/Windows driver combinations (e.g. 780M) it passes the probe but then deadlocks on the real encode, so AMD-on-Windows uses libx264 (all cores); AMD-on-Linux still gets VAAPI. As a safety net, if any selected hardware encoder stalls on the real encode, it is automatically abandoned and retried on libx264, so an export never hangs forever.
